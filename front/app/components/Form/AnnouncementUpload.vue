<template>
  <Dialog
    ...
    aria-labelledby="announcement-upload-title"
  >
        ...
        >
          <div class="flex min-w-0 gap-4">
            ...
            <div class="min-w-0">
              ...
              <h2
                id="announcement-upload-title"
                class="mt-1 truncate text-xl font-bold leading-tight text-white sm:text-2xl"
              >
                {{ announcement.title || 'Upload image' }}
              </h2>
              <p class="mt-2 text-sm leading-6 text-slate-300">
                Upload the image shown with this announcement on the public site.
              </p>
            </div>
          </div>
          ...
          <Button
            ...
            text
            severity="secondary"
            aria-label="Close announcement upload"
                  ...
                  <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-slate-600 dark:text-surface-300">
                        Use a clear JPG, PNG, or WebP under 1 MB.
                      </p>
                    </div>
              ...
              <template #empty>
                ...
                >
                  ...
                  <p class="mt-2 max-w-sm text-sm leading-6 text-slate-600 dark:text-surface-300">
                    Or use the image button above to choose the announcement image.
                  </p>
                </div>
              </template>
            </FileUpload>
...
const selectedSizeLabel = computed(() => {
  ...
  return `${formatSize(totalSize.value)} selected`
})
...
async function processUpload() {
  ...
  await task()
    .do(async () => useLoaderStore().processStep('announcementUpload'))
    .do(async () => {
